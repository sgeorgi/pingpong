...
describe 'layouts/_top_bar.html.slim' do
  ...
  it 'shows a link to login' do
    skip 'Feature :user_login is disabled' unless FeatureToggle.on?(:user_login)
    render
  ...
  it 'does not show a link to profile when not logged in' do
    skip 'Feature :user_login and :user_profile are disabled' unless FeatureToggle.on?(:user_login) and FeatureToggle.on?(:user_profile) do
      allow(view).to receive(:user_signed_in?).and_return(false)
      render
  ...
  it 'does show a link to profile when logged in as user' do
    skip 'Feature :user_login and :user_profile are disabled' unless FeatureToggle.on?(:user_login) and FeatureToggle.on?(:user_profile)
    allow(view).to receive(:user_signed_in?).and_return(true)
    render
